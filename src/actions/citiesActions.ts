"use server";
import { CityObject } from "@/types/projectTypes";
import { prisma } from "@/utils/prismaClient";
import { incrementStatistics } from "./statisticsActions";

const api_key = process.env.NEXT_PUBLIC_OPEN_WEATHER_API_KEY;

export const addNewCity = async (cityObject: CityObject) => {
  try {
    const existingCity = await prisma.city.findUnique({
      where: {
        name: cityObject.name,
      },
    });

    if (existingCity) {
      console.log("City already exists, skipping creation.");
      return;
    }

    await prisma.city.create({
      data: {
        name: cityObject.name,
        country: cityObject.country,
        latitude: cityObject.lat,
        longitude: cityObject.lon,
        searchCount: 1,
      },
    });
  } catch (error) {
    console.error("Error in addNewCity:", error);
    return { message: "Error while adding city" };
  }
};

export const searchCity = async (cityName: string, limit: number = 5) => {
  try {
    const response = await fetch(
      `http://api.openweathermap.org/geo/1.0/direct?q=${cityName}&limit=${limit}&appid=${api_key}`,
    );

    const data = await response.json();
    return data;
  } catch (error: any) {
    console.error(error);
    throw new Error("Error while searching for cities");
  }
};

export const getCityWeather = async (city: CityObject) => {
  try {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?lat=${city.lat}&lon=${city.lon}&appid=${api_key}&units=metric`,
    );
    if (response.ok) {
      await incrementStatistics(city);
    }
    const data = await response.json();

    return data;
  } catch (error: any) {
    console.error(error);
    throw new Error("Error while fetching city weather");
  }
};
